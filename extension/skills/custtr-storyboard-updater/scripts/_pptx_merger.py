...
def _media_mime_for(ext):
    ...
    to octet-stream is risky â€” PowerPoint may refuse to embed media without
    a recognised type â€” so the curated table should be kept current as new
...
# 256 <= id < 2^31 (ECMA-376 Â§19.2.1.40-42). Naming these makes the
# allocator code self-documenting and the call sites grep-able.
SLIDE_ID_MIN          = 256
SLIDE_MASTER_ID_MIN   = 0x80000000   # 2_147_483_648 â€” schema lower bound for sldMasterId
...
def resolve(base, target):
    ...
    than the path has depth) are silently dropped â€” the previous behaviour
...
def preflight_source(path, max_bytes=None):
    ...
    Successful pre-flight returns the file's size in bytes â€” useful for
...
class Merger:
    def __init__(self, base_path):
        ...
        # `_rels/` siblings â€” counting both inflates the prefix counter and
        ...
        # Capture base slide size â€” every merged source MUST match this. PPTX
        ...
        # Note: avoid `or []` on the lxml element â€” element truthiness is
        ...
        # whole package and must all be unique (ECMA-376 Â§19.2.1.40-42). Start the
        ...
        # Strip changesInfos â€” author-specific metadata, invalid in a merged context;
        ...
        # Same problem for customXml â€” base file's items are bulk-loaded.
    ...
    def _warn(msg):
        ...
        bite later) â€” silent `except: continue` hides bugs.
        """
        log.warning(msg)
    ...
    def _route_dep(self, src, src_res, dest_parent, processed, remap,
        ...
        `remap`            â€” src_path -> dest_path cache shared across one
                             source-merge call. Used both for dedup and as
                             the resolution source for slideMaster, theme,
                             and other cross-references.
        `src_ct_overrides` â€” content-type Overrides parsed from the source's
        ...
        if re.match(r"customXml/item\d+\.xml$", src_res):
            ...
            # from presentation.xml.rels â€” without this, PowerPoint flags the
        ...
        if re.match(r"ppt/diagrams/", src_res):
            ...
            # quickStyle, drawing). Slides reference each part directly â€”
            # there is no "primary" part whose recursion drags in the
            # rest â€” so we can't group them under a shared prefix without
    ...
    def _copy_layout(self, src, src_layout, src_master, dest_master,
        ...
        removes a real drift risk â€” any future fix to _copy_recursive now
    ...
    def _strip_reaction_extlst(self, dest_part):
        ...
        for extlst in root.iter(f"{{{P188_NS}}}extLst"):
            ...
            # (those are the schema-flagged ones â€” purely reactions in
    ...
    def _merge_author_registry(self, src, src_path, base_path, id_attr,
        ...
        - For modern co-authoring (`ppt/authors.xml`, p188:authorLst), id is
          a GUID â€” globally unique by design, so simple dedup is correct.
        ...
        if base_path not in self.parts:
            # Base lacks this registry â€” copy source's wholesale and register
        ...
        if collisions:
            log.warning(
                "%d legacy author id collisions in %s (skipped â€” comments "
    ...
    def merge_file(self, src_path, wanted=None):
        ...
        # rather than stashed on `self` â€” that pattern caused subtle
    ...
    def _audit_repair_risks(self):
        ...
        #    children (Sch_UndeclaredAttribute â†’ Repair).
        offending = []
        ...
        #     and is purged at save â€” leaving body refs pointing nowhere.
    ...
    def save(self, output_path):
        ...
        # the audit itself â€” that produced duplicate lines when the CLI
        ...
        # Use default namespace (no prefix) â€” Open XML SDK requires <Types xmlns="..."> not <ns0:Types>
...
def _build_arg_parser():
    ...
    p.add_argument(
        ...
        help="Validate, merge, audit â€” but don't write the output file",
    )
    p.add_argument(
...
def main(argv=None):
    ...
    try:
        ...
        log.info("done â€” %d slides, audit errors=%d warnings=%d",
        ...
        # logged inside the Merger â€” surface clean exit code 2.
        log.error("%s", e)
        result["error"] = str(e)
        result["duration_seconds"] = round(time.monotonic() - started, 3)
...
def merge(sources, output_path):
    return main([str(output_path), *[str(s) for s in sources]])
