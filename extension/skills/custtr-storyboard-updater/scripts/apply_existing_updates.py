...
from ooxml_helpers import NS, q, parse, out_xml, ordered_slides, shape_text, text_shapes, validate_pptx  # noqa: E402
from json_helpers import safe_load_json  # noqa: E402


def _force_dark_fill(rpr):
    """On a highlighted run, always force dark text (tx1) regardless of original fill.

    The original fill may be explicit white (srgbClr=FFFFFF) or the bg1 scheme token.
    Both are invisible on a yellow highlight background, so we unconditionally replace
    any existing solidFill with tx1 on highlighted runs only.
    """
...
def add_highlight(rpr):
    """Inject <a:highlight val=FFFF00> in the correct OOXML position and force dark text."""
...
def fragment_replace(root, find_fragment, replace_fragment):
    """Surgical in-place replacement of a text fragment within a single <a:t> run.

    Finds the first <a:r> whose <a:t> contains find_fragment, then splits that run
    into: text_before + [char-level diff runs for find→replace] + text_after.
    All new runs inherit the original run's <a:rPr> exactly, so font, size, and
    color are preserved on unchanged text. Only the changed characters get yellow
    highlight. The run's parent <a:p> is modified in-place; nothing else changes.

    This avoids the phantom-newline problem caused by shape_text() joining multiple
    <a:t> elements across runs — it operates on the individual <a:t> level.
    """
        ...
        for p in sp.findall(".//" + q("a", "p")):
            for r in list(p.findall(q("a", "r"))):
                ...
                if t_el is None or t_el.text is None:
                    continue
                if find_fragment not in t_el.text:
                    continue

                original_text = t_el.text
                rpr_orig = r.find(q("a", "rPr"))
                rPr_tmpl = copy.deepcopy(rpr_orig) if rpr_orig is not None else ET.Element(q("a", "rPr"), {"lang": "en-US"})
                for old_hl in list(rPr_tmpl.findall(q("a", "highlight"))):
                    rPr_tmpl.remove(old_hl)
                rPr_tmpl.attrib.pop("strike", None)

                idx = original_text.index(find_fragment)
                prefix = original_text[:idx]
                suffix = original_text[idx + len(find_fragment):]

                # Collect sibling runs to splice into
                siblings = list(p)
                r_idx = siblings.index(r)
                p.remove(r)

                new_runs = []

                def _run(text, highlight=False, strike=False):
                    nr = ET.Element(q("a", "r"))
                    rpr = copy.deepcopy(rPr_tmpl)
                    if strike:
                        rpr.set("strike", "sngStrike")
                ...
                if prefix:
                    new_runs.append(_run(prefix))

                # Character-level diff of just the changed fragment
                for tag, i1, i2, j1, j2 in difflib.SequenceMatcher(
                    None, find_fragment, replace_fragment, autojunk=False
                ).get_opcodes():
                    if tag == "equal":
                        new_runs.append(_run(replace_fragment[j1:j2]))
                    elif tag == "delete":
                        new_runs.append(_run(find_fragment[i1:i2], highlight=True, strike=True))
                    elif tag == "insert":
                        new_runs.append(_run(replace_fragment[j1:j2], highlight=True))
                    elif tag == "replace":
                        new_runs.append(_run(find_fragment[i1:i2], highlight=True, strike=True))
                        new_runs.append(_run(replace_fragment[j1:j2], highlight=True))

                if suffix:
                    new_runs.append(_run(suffix))

                for offset, nr in enumerate(new_runs):
                    p.insert(r_idx + offset, nr)

                return True  # only replace first occurrence
    return False
