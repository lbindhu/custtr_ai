#!/usr/bin/env python3
"""Build an advisory variant × coverage parity lint matrix from deck_extract.json.
...
detail bullets, and knowledge-check slides. Emits advisory signals for variants
with zero dedicated slides when peers have >=1.

Important: this script is a lint signal generator, not a semantic judge. The
LLM-authored audit must confirm, reject, or refine every signal before any plan
action is created.
    ...
    for v in variants:
        ...
        if dedicated_count == 0 and peers_with_slides:
            ...
            candidates.append({
                ...
                "visual_intent": "Create a dedicated teaching slide with visual depth comparable to peer variants.",
...
def main() -> None:
    ...
    if not variants:
        print(json.dumps({
            ...
            "coverage_gaps": [],
            "advisory": True,
            "_note": "No product_variants in stale_terms.json and no --variant-pattern supplied; skipping advisory parity lint.",
        ...
        Path(args.output).write_text(
            json.dumps({
                ...
                "coverage_gaps": [],
                "advisory": True,
                "_note": "No product_variants configured; advisory parity lint skipped.",
            }, indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
        return
    ...
    result = {
        ...
        "coverage_gaps": gaps,
        "advisory": True,
        "llm_disposition_required": bool(candidates or gaps),
    }
    ...
    if candidates:
        print(
            f"\nADVISORY: {len(candidates)} variant(s) with 0 dedicated slides while peers have ≥1. "
            "The LLM must confirm, reject, or refine these signals before planning actions.",
            file=sys.stderr,
        )
