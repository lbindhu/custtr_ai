        ...
        for index, finding in enumerate(row.get("findings") or []):
            ...
            if ftype == "new_slide_candidate" or recommended == "add_new_slide":
                ...
                actions.append({
                    ...
                    "slide_layout": finding.get("recommended_layout") or "ascii_diagram",
...
def first_slide_by_role(story, role):
    for item in story.get("slide_roles", []):
...
def slides_by_role(story, role):
    return [item["slide_number"] for item in story.get("slide_roles", []) if item.get("role") == role]
...
def source_delta_actions(deltas, story):
    ...
    checks = story.get("knowledge_checks", [])
    deep_dive_slides = [
        item["slide_number"]
        for item in story.get("slide_roles", [])
        if item.get("role") in {"deep_dive", "concept_setup", "comparison", "recommendation"}
    ]
