...
verbs, etc.) and check whether they're covered by any entry in
reference_extract.json. Emits coverage_gaps.json listing the slides and claims
that have no source coverage.
...
Exit codes:
  0  every slide is fully covered (no gaps)
  1  one or more slides have uncovered claims — recursive source gathering required
  2  hard error (missing inputs, malformed JSON)

The planner is expected to iterate: feed the uncovered claims to MCP/web
fetchers, extend reference_extract.json with new entries (each tagged with
`claim_keys: [...]`), and re-run this script until every slide is fully
covered (or escalate unresolved gaps to the user).
"""
...
# reference corpus. The tokens are intentionally specific enough that a
# match in the source text means the source actually discusses that fact.
