    ...
    for src_file in all_files:
        try:
            ...
            if isinstance(data, list):
                ...
                # A batch file must be a top-level JSON array. A common mistake
                # is wrapping rows as {"slides": [...]} — recover by unpacking
                # but warn loudly so the author fixes the envelope (B-006).
                if isinstance(data.get("slides"), list):
                    errors.append(
                        f"{src_file.name}: batch file is wrapped as "
                        f"{{'slides': [...]}}; expected a top-level JSON array "
                        f"of slide rows. Unpacking 'slides' — fix the envelope."
                    )
                    for item in data["slides"]:
                        if isinstance(item, dict):
                            rows.append(item)
                        else:
                            errors.append(f"{src_file.name}: 'slides' item is {type(item).__name__}, expected object")
                elif "slide_number" in data:
                    # Legacy single-slide sidecar (sNN_verification.json).
                    rows.append(data)
                else:
                    errors.append(
                        f"{src_file.name}: dict has no 'slide_number' and no "
                        f"'slides' array; expected a single slide row "
                        f"({{'slide_number': N, ...}}) or a JSON array of rows"
                    )
            else:
