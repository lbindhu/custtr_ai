...
def pptx_slide_count(path: str | Path) -> int:
    with zipfile.ZipFile(path) as z:
        return sum(1 for name in z.namelist() if re.fullmatch(r"ppt/slides/slide\d+\.xml", name))
...
def main():
    ...
    ap.add_argument(
        "--additions",
        nargs="*",
        help="One or more LLM-produced additions PPTX files, in plan action order",
    )
    ...
    addition_sources = []
    for src in args.additions or []:
        count = pptx_slide_count(src)
        for idx in range(1, count + 1):
            addition_sources.append((src, idx))
    if len(addition_sources) < len(add_actions):
        raise SystemExit(
            f"ERROR: plan has {len(add_actions)} add_new_slide action(s), but "
            f"the provided additions deck(s) contain {len(addition_sources)} slide(s)."
        )

    # Build insertion groups: list of (insert_after_slide, source_file, slide_index_in_source)
    insertions = []
    for action, (src, idx) in zip(add_actions, addition_sources):
        insertions.append((action.get("insert_after_slide", slide_count), src, idx))
