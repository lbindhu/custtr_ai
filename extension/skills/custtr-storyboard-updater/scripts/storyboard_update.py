...
def _run_status(deck_hint: str = "") -> None:
    ...
    if extract.exists():
        ...
        print(f"Phase 1   [DONE]  deck_extract.json ({slide_count} slides), story_model.json")
    else:
    ...
    if src_inv.exists():
        ...
        queries = src_data.get("queries") or []
        nabu = sum(1 for q in queries if q.get("server") == "nabu")
        conf = sum(1 for q in queries if q.get("server") in {"confluence", "atlassian"})
        jira = sum(1 for q in queries if q.get("server") == "jira")
        print(f"Phase 2   [DONE]  source_inventory.json ({len(queries)} sources: {nabu} NABU, {conf} Confluence, {jira} JIRA)")
    else:
    ...
    if gaps.exists():
        ...
        open_gaps = len([g for g in (gaps_data.get("gaps") or []) if not g.get("resolved")])
        parity_tag = ", parity_matrix.json" if parity.exists() else ""
        print(f"Phase 2.5 [DONE]  coverage_gaps.json ({open_gaps} open gaps){parity_tag}")
    else:
    ...
    # Phase 5
    validation = wd / "validation_report.json"
    if validation.exists():
        print(f"Phase 5   [DONE]  validation_report.json")
    ...
    if sidecars and not vr.exists():
        ...
        print("Next step: Complete Phase 2 — query NABU, Confluence, JIRA and save source_inventory.json.")
...
def main():
    ...
    ap.add_argument("--require-sources", action="store_true",
                    help="Pass through to audit_gate.py — enforces source_inventory minimums.")
    ...
    if args.mode in {"audit-plan", "audit-only"}:
        ...
        run([sys.executable, HERE / "build_story_model.py", "--deck-extract", extract, "--output", story_model])
        ...
        if args.mode == "audit-only":
            ...
            return
        build_cmd = [
            sys.executable, HERE / "build_update_plan.py",
            "--deck-extract", extract,
        ...
        if args.source_deltas:
            build_cmd.extend(["--source-deltas", args.source_deltas])
        run(build_cmd)
        print(f"Field-name reference (read before authoring ANY JSON): {HERE.parent / 'references' / 'artifact_schemas.md'}")
        print(f"Plan created and requires approval: {plan}")
        print(f"Story model: {story_model}")
        print(f"Mechanical scans: {wd / 'stale_term_scan.md'}, {wd / 'scope_consistency.json'}")
        print(f"Parity matrix: {wd / 'parity_matrix.json'}")
        print(f"Concept decomposition (author before audit_gate): {wd / 'concept_decomposition.json'}")
        print("Before approving, author concept_decomposition.json, then run audit_gate.py to "
              "confirm every finding is either addressed in the plan or explicitly cleared "
              "in cross_validation_report.json.")
        return

    if not args.approved_plan:
        raise SystemExit("--approved-plan is required in execute mode")

    updated_base = wd / f"{deck.stem}_existing_updates.pptx"
    additions = wd / "generated_additions.pptx"
    final = Path(args.output) if args.output else deck.with_name(f"{deck.stem}_{args.target_version or 'updated'}_updated.pptx")
    validation = wd / "validation_report.json"
    ...
    if any(a.get("type") == "add_new_slide" for a in plan_data.get("actions", [])):
        run([sys.executable, HERE / "create_additions_deck.py", "--plan", plan, "--output", additions])
    ...
    run(cmd)
    run([sys.executable, HERE / "validate_deck.py", final, "--plan", plan, "--output", validation, "--work-dir", str(wd)])
