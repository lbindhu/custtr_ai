...
def main():
    ...
    with zipfile.ZipFile(args.deck) as z:
        ...
        if args.plan:
            ...
            new_count = len([a for a in plan.get("actions", []) if a.get("type") == "add_new_slide"])
            if new_count:
                found_badges = 0
                highlighted_new = 0
                for path in order:
                    vals = texts(z, path)
                    if "New Slide" in vals:
                        found_badges += 1
                        if b"<a:highlight" in z.read(path):
                            highlighted_new += 1
                if found_badges < new_count:
                    report["errors"].append(f"expected at least {new_count} New Slide badges, found {found_badges}")
                if highlighted_new:
                    report["errors"].append(f"{highlighted_new} new slide(s) contain highlight markup")
