...
class TestMergeSlideRows:
    ...
    def test_wrong_slides_envelope_is_unpacked(self, tmp_path):
        # B-006: a batch file wrongly wrapped as {"slides": [...]} must be
        # unpacked into individual rows, not appended as one bogus row.
        rows_dir = tmp_path / "slide_rows"
        rows_dir.mkdir(parents=True)
        (rows_dir / "batch_01.json").write_text(
            json.dumps({"slides": [
                {"slide_number": 1, "findings": []},
                {"slide_number": 2, "findings": []},
            ]}),
            encoding="utf-8",
        )
        out = tmp_path / "verification_report.json"
        merge(tmp_path, out)
        report = json.loads(out.read_text())
        assert [r["slide_number"] for r in report["slides"]] == [1, 2]
